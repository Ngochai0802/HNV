...
export default function PatientLayout() {
  ...
  // Fetch số thông báo chưa đọc — re-fetch mỗi khi đổi trang
  useEffect(() => {
    if (!user) return;
    getNotifications()
      .then((res) => {
        ...
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between shadow-sm">
          ...
          {user && (
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/patient/notifications"
                className="relative w-9 h-9 rounded-full bg-slate-100 hover:bg-blue-50 flex items-center justify-center text-slate-500 hover:text-blue-600 transition-all duration-200"
                title="Thông báo"
              >
                <Bell size={18} />
                {unreadCount > 0 && (
