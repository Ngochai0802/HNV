  ...
  useEffect(() => {
    getConversations()
      .then((res) => {
        ...
        if (res.data.length > 0) loadMessages(res.data[0].id);
      })
      .catch(() => {});
  }, []);
          ...
          {conversations.length === 0 ? (
            <div className="text-center py-10 px-4">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Bệnh nhân sẽ liên hệ khi cần tư vấn
              </p>
            </div>
          ) : (
            conversations.map((c) => {
          ...
          <>
              ...
              {messages.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!isMe && (
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isAI ? "bg-purple-100" : "bg-blue-100"}`}
                      >
                        {isAI ? (
                    ...
                    >
                      ...
                      {!isMe && (
                        <p
                          className={`text-xs font-semibold mb-1 ml-1 ${isAI ? "text-purple-500" : "text-blue-600"}`}
                        >
                    ...
                    {/* Avatar bên phải cho bác sĩ (mình) */}
                    {isMe && (
            ...
            <div className="p-4 border-t border-slate-100 flex gap-3 bg-white">
              <input
                ...
                className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition"
              />
              <button
                onClick={handleSend}
                disabled={loading || !input.trim()}
                className="w-11 h-11 bg-teal-600 text-white rounded-xl flex items-center justify-center hover:bg-teal-700 transition disabled:opacity-50 shadow-sm"
              >
                {loading ? (
