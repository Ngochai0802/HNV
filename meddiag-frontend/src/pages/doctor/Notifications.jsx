        ...
        notifications.map((n) => (
          <div
            ...
            className={`bg-white rounded-2xl border p-5 cursor-pointer transition hover:shadow-md
              ${n.isRead ? "border-slate-100" : "border-teal-200 bg-teal-50"}`}
          >
            <div className="flex items-start gap-3">
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0
                ${n.isRead ? "bg-slate-100 text-slate-400" : "bg-teal-100 text-teal-600"}`}
              >
                <Bell size={18} />
              </div>
              ...
              {!n.isRead && (
                <div className="w-2.5 h-2.5 bg-teal-600 rounded-full flex-shrink-0 mt-1" />
              )}
            </div>
          </div>
        ))
