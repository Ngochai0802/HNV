...
export default function DoctorNotifications() {
  ...
  const handleNotificationClick = async (n) => {
    ...
    if (!url) {
      ...
      } else if (lowerTitle.includes("phân công") || lowerTitle.includes("ảnh") || lowerTitle.includes("kết quả")) {
        url = "/doctor/cases";
      }
    }
  ...
  return (
    <div className="space-y-6 w-full pb-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 transition focus-within:border-teal-400 focus-within:ring-2 focus-within:ring-teal-100">
            ...
            {paginatedNotifications.map((n) => (
              <div
                ...
                className={`group rounded-2xl border p-5 cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1
                  ${n.isRead ? "bg-white border-slate-100" : "bg-gradient-to-r from-teal-50/50 to-white border-teal-200"}`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors
                    ${n.isRead ? "bg-slate-50 text-slate-400 group-hover:bg-slate-100" : "bg-teal-100 text-teal-600 group-hover:bg-teal-200"}`}
                  >
                    <Bell size={22} />
                  </div>
                  ...
                  {!n.isRead && (
                    <div className="w-3 h-3 bg-teal-500 rounded-full flex-shrink-0 mt-3 shadow-sm" />
                  )}
                </div>
              </div>
            ))}
