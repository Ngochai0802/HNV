...
export default function PatientChat() {
  ...
  const caseContextMsg = [...messages].reverse().find(
    (m) => m.senderId === user?.id && m.content?.startsWith("🩻")
  );
                ...
                return (
                  ...
                  >
                    ...
                    {!isMe && (
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0
                          ${isAI ? "bg-purple-100" : "bg-teal-100"}`}
                      >
                        {isAI ? (
                    ...
                    <div className={`max-w-xs lg:max-w-md ${isMe ? "order-1" : ""}`}>
                      {/* Tên người gửi */}
                      {!isMe && (
                        <p className={`text-xs font-semibold mb-1 ml-1
                          ${isAI ? "text-purple-500" : "text-teal-600"}`}>
                      ...
                      {/* Thời gian */}
                      <p className={`text-[10px] mt-1 ${isMe ? "text-right mr-1" : "ml-1"} text-slate-400`}>
