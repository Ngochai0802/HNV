...
export default function Upload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Kiểm tra AI có reject ảnh không
      if (res.data.aiStatus === "failed" || res.data.error) {
        toast.error(res.data.error || "Ảnh không phải X-quang phổi, vui lòng thử lại!");
        setFile(null);
        setPreview(null);
        setProgress(0);
        return;
      ...
      toast.error(err.response?.data?.message || "Ảnh không phải X-quang phổi, upload lại");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="bg-blue-950 rounded-3xl p-6 text-white shadow-xl shadow-blue-900/20 border border-blue-800 relative overflow-hidden">
            ...
            <ul className="space-y-4 text-sm text-blue-100/80 font-medium relative z-10">
              ...
              <li className="flex gap-3">
                ...
                <span>Dung lượng tập tin: <strong className="text-white">Tối đa 20MB</strong></span>
              </li>
              <li className="flex gap-3">
